...
for(i in 1:nrow(samples)) {
  cat(paste0("zcat ",
             ...
             as.vector(samples$LibraryName)[i],"_L003_R1_001.fastq.gz ",
             as.vector(samples$LibraryName)[i],"_L004_R1_001.fastq.gz > ",
...
for(i in 61:nrow(samples)) {
  
  #remove low quality reads
  cat(paste0("TrimmomaticSE -threads ",threads,
             " -phred33",
             " raw/",as.vector(samples$SampleName)[i],"_1.fastq.gz",
